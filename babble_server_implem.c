...
int server_connection_init(int port) {
  ...
  if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (void *)&reuse_opt,
                 sizeof(reuse_opt)) < 0 &&
      setsockopt(sockfd, SOL_SOCKET, SO_REUSEPORT, (void *)&reuse_opt,
                 sizeof(reuse_opt)) < 0) {
...
int run_login_command(command_t *cmd, answer_t **answer) {
  ...
  snprintf(msg_buffer, BABBLE_BUFFER_SIZE, "%s[%ld]: registered with key %lu\n",
           client_data->client_name, tt.tv_sec - server_start,
           client_data->key);
...
static void generate_timeline_summary(unsigned long client_key,
                                      timeline_item_t *pub_list,
                                      int timeline_size, answer_t **answer) {
  answer_t *the_answer = NULL;
  ...
  while (time_iter != NULL) {
    if (remaining <= BABBLE_TIMELINE_MAX) {
      snprintf(msg, BABBLE_BUFFER_SIZE, "    %s[%ld]: %s\n",
               time_iter->client->client_name, time_iter->pub->date,
               time_iter->pub->msg);
...
int run_timeline_command(command_t *cmd, answer_t **answer) {
  ...
  for (i = 0; i < client->nb_followed; i++) {
    ...
    while ((pub = publication_set_getnext(f_client->pub_set, pub,
                                          start_time)) != NULL) {
